...
This project uses Databricks AppKit packages. For AI assistant guidance on using these packages, refer to (available **after** the first `./start.sh` / `npm ci` installs deps — `node_modules/` is not shipped with the template):
